...
with open("params.h", "w") as f:
    ...
    for name, tensor in state_dict.items():
        ...
        # -----------------------------------------
        # Export BatchNorm as scale + shift
        # -----------------------------------------
        ...
        if name.endswith(".weight"):
            ...
            if prefix in bn_prefixes:
                ...
                f.write(
                    f"static const float {cname}_scale[{len(scale)}] = "
                )
                ...
                f.write(
                    f"static const float {cname}_shift[{len(shift)}] = "
                )
        ...
        f.write(
            f"static const float {cname}{dims} = "
        )
