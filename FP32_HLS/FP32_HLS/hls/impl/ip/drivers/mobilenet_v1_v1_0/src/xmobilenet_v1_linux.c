...
typedef struct {
    ...
    XMobilenet_v1_uio_map maps[ MAX_UIO_MAPS ];
} XMobilenet_v1_uio_info;

/***************** Variable Definitions **************************************/
static XMobilenet_v1_uio_info uio_info;
...
static int uio_info_read_name(XMobilenet_v1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMobilenet_v1_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMobilenet_v1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMobilenet_v1_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMobilenet_v1_Initialize(XMobilenet_v1 *InstancePtr, const char* InstanceName) {
	XMobilenet_v1_uio_info *InfoPtr = &uio_info;
