...
static int inference(XMobilenet_v1_qat *ip, int image_index) {
    ...
    XMobilenet_v1_qat_Set_input_r(ip, img_addr);
    XMobilenet_v1_qat_Set_output_r(ip, out_addr);
    XMobilenet_v1_qat_Start(ip);
    
    while (!XMobilenet_v1_qat_IsDone(ip));
...
int main(void) {
    XMobilenet_v1_qat mobilenet_v1_qat_ip;
    uint64_t tStart, tEnd;

    Xil_Out32(GLOBAL_TMR_CTRL, 0x1);

    xil_printf("========================================\r\n");
    xil_printf("MobileNetV1-QAT Inference\r\n");

    // Khởi tạo IP
    int status = XMobilenet_v1_qat_Initialize(&mobilenet_v1_qat_ip, XPAR_MOBILENET_V1_QAT_0_BASEADDR);
    ...
    for (int i = 0; i < NUM_SAMPLES; i++) {
        int predict = inference(&mobilenet_v1_qat_ip, i);
