...
`ifndef MOBILENET_V1_QAT_SUBSYSTEM_MONITOR_SV
`define MOBILENET_V1_QAT_SUBSYSTEM_MONITOR_SV
...
class mobilenet_v1_qat_subsystem_monitor extends uvm_component;

    mobilenet_v1_qat_reference_model refm;
    mobilenet_v1_qat_scoreboard scbd;

    `uvm_component_utils_begin(mobilenet_v1_qat_subsystem_monitor)
    `uvm_component_utils_end

    uvm_analysis_imp_axi_wtr_gmem#(axi_pkg::axi_transfer, mobilenet_v1_qat_subsystem_monitor) gmem_wtr_imp;
    uvm_analysis_imp_axi_rtr_gmem#(axi_pkg::axi_transfer, mobilenet_v1_qat_subsystem_monitor) gmem_rtr_imp;
    uvm_analysis_imp_axi_wtr_CTRL_BUS#(axi_pkg::axi_transfer, mobilenet_v1_qat_subsystem_monitor) CTRL_BUS_wtr_imp;
    uvm_analysis_imp_axi_rtr_CTRL_BUS#(axi_pkg::axi_transfer, mobilenet_v1_qat_subsystem_monitor) CTRL_BUS_rtr_imp;

    virtual function void build_phase(uvm_phase phase);
        super.build_phase(phase);
        if (!uvm_config_db#(mobilenet_v1_qat_reference_model)::get(this, "", "refm", refm))
            `uvm_fatal(this.get_full_name(), "No refm from high level")
        `uvm_info(this.get_full_name(), "get reference model by uvm_config_db", UVM_MEDIUM)
        scbd = mobilenet_v1_qat_scoreboard::type_id::create("scbd", this);
    endfunction
